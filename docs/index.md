...
Автор: **AlexFirst** · Версия: **1.10.0** · Репозиторий и релизы: <https://github.com/AlexFirst404/alexmenus-editor>
...
!!! danger "Что могло сломаться в 1.10.0"
    ...
    **2. Меню с `permission:` больше не открывается через `open_menu`.** До 1.10.0 право гейтило только
    ...
    [`open-requirement`](Requirements.md) или `view-requirement` на кнопку-переход. При отказе на пути
    `open_menu` игрок **не получает сообщения** — окно просто не открывается. См.
    [Commands-and-Permissions](Commands-and-Permissions.md).

    **3. `type: none` в `open-animation` больше не «включается» из-за оставшегося `order:`** (исправлено в
    1.9.0). Если ты выключал анимацию через `type: none`, но не убирал `order:`, анимация молча продолжала
    играть как `custom`. Теперь она действительно выключена. Чтобы вернуть — `type: custom`. См.
